...
type Collector interface {
	Start() error
	Stop() error
	Packets() <-chan packets2.PacketSummary
}
...
type collectorBase struct {
	...
	PacketCount int    //running count of observed comm
	cappoints   []*capture.CapturePoint
	pipeline    chan packets2.PacketSummary
	mapperconn  packets2.PacketCollectionClient
...
func (c *collectorBase) Connect(remote net.IPAddr) error {
	...
	c.mapperconn = packets2.NewPacketCollectionClient(conn)
	return nil
}
...
func (c *collectorBase) transmit() (err error) {
	...
	select {
	case p = <-c.pipeline:
		go func(p packets2.PacketSummary) {
			msg := &packets2.PacketSummaryMessage{
				CapID:  p.CapID,
